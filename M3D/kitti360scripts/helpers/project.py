...
class Camera:
    def __init__(self):
        ...
        print(f"zl-tst cam2world: {self.cam2world[1.0].shape}, {len(self.cam2world)}, {self.cam2world.keys()}")
        # self.frame2idx = self.get_frame_to_kitti_index_dict() 
        self.write2txt(self.cam2world)
    ...
    def world2cam(self, points, R, T, inverse=False):
        ...
        # print(f"points: {points.shape}-{points}") # zltst
        if not inverse:
    ...
    def project_vertices(self, vertices, frameId, inverse=True):
        # def project_vertices(self, vertices, frameId, inverse=False):  # ---zltst---

        # current camera pose
        # while frameId not in self.cam2world:
        #     frameId -= 1  # zltst
        ...
        print(f"points_local: {points_local.shape}-{points_local}")
    ...
    def get_frame_to_kitti_index_dict(self):
    ...
    def write2txt(self, data_dict, output_dir="/data2/KITTI-360/Ext_cam2world"):
        os.makedirs(output_dir, exist_ok=True)

        # 处理字典
        for key, value in data_dict.items():
            # 将键格式化为 6 位数字字符串，不足部分左侧补 0
            # seq_frame = f"{self.seq}-{int(key):010d}"
            # print(f"seq_frame:{seq_frame}")
            # if seq_frame not in frame2idx:
            #     print(f"{seq_frame}, not available.")
            #     continue 
            # file_name = f"{int(frame2idx[seq_frame]):06d}.txt"
            file_name = f"{int(key):010d}.txt"
            file_dir = os.path.join(output_dir, self.seq)
            if not os.path.exists(file_dir):
                os.makedirs(file_dir)
            file_path = os.path.join(file_dir, file_name)
            print(f"file_path:{file_path}")
            
            # 将 4x4 数组 reshape 为 1x16 数组
            value_reshaped = value.reshape(1, 16)
            
            # 将数组写入文件
            with open(file_path, 'w') as f:
                # 将每个数值格式化为字符串，并用空格分隔
                row_str = ' '.join(f"{num:.6f}" for num in value_reshaped[0])
                f.write(row_str + '\n')
...
class CameraPerspective(Camera):
    ...
    def cam2image(self, points):
        ...
        print(f"self.K: {self.K.shape} {self.K}")  # zltst
        print(f"points: {points.shape} {points}")  # zltst
        points_proj = np.matmul(self.K[:3,:3].reshape([1,3,3]), points)
        
        # print(f"points_proj: {points_proj.shape}-{points_proj}")  # zltst
        depth = points_proj[:,2,:]
        depth[depth==0] = -1e-6
        u = np.round(points_proj[:,0,:]/np.abs(depth)).astype(np.int16)
        v = np.round(points_proj[:,1,:]/np.abs(depth)).astype(np.int16)
