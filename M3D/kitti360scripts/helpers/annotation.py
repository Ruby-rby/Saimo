...
def global2local(globalId):
    ...
    if isinstance(globalId, np.ndarray):
        return semanticId.astype(np.int16), instanceId.astype(np.int16) # zltst 16
    else:
...
class Annotation3D:
    ...
    def __init__(self, labelDir='', sequence=''):
        ...
        labelPath = glob.glob(os.path.join(labelDir, 'train', '%s.xml' % sequence)) # train or test # zltst
        # print("labelPath: ", labelPath) # zltst
        if len(labelPath)!=1:
