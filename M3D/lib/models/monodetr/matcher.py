...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, group_num=11):
        ...
        # print(f'zl-tst pred_boxes:{outputs["pred_boxes"].shape} outputs["pred_boxes"]:{outputs["pred_boxes"]} out_bbox:{out_bbox}')
        ...
        # Final cost matrix
        # cost_class：分类误差 [batch_size * num_queries, num_target_boxes， self.cost_bbox：权重
        # cost_3dcenter 3D中心位置误差 [batch_size * num_queries, num_target_boxes]
        # cost_bbox 2D边界框误差 [batch_size * num_queries, num_target_boxes]
        # cost_giou 广义交并比 [batch_size * num_queries, num_target_boxes]
        try: 
            C = self.cost_bbox * cost_bbox + self.cost_3dcenter * cost_3dcenter + self.cost_class * cost_class + self.cost_giou * cost_giou
        except Exception as e:
            print(f"zl-tst cost_bbox {cost_bbox.shape} {cost_bbox.device}")
            print(f"zl-tst cost_3dcenter {cost_3dcenter.shape} {cost_3dcenter.device}")
            print(f"zl-tst cost_bbox {cost_bbox.shape} {cost_bbox.device}")
            print(f"zl-tst cost_giou {cost_giou.shape} {cost_giou.device}")
            raise e
